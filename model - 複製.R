...
TrainingModel <- function(model_name, train_data)
{
    ...
    # res_cm <- addmargins(table(res_predictions, train_data$Survived))
    
    # Print the results
    message(sprintf("======= %s Model =======", model_name))
    cat("使用 training data 進行預測的準確率：", res_accuracy , '\n')
    ...
    # Plot ROC curve
    par(pty="s")
    plot(roc_obj, main = paste("ROC Curve"), col = "blue", legacy.axes = TRUE, print.auc = TRUE)
